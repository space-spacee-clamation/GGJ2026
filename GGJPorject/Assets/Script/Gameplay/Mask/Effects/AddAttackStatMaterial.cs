...
/// <summary>
/// 战斗中对运行时 Attack 做加值（可为负数）。
/// </summary>
public sealed class AddAttackStatMaterial : MonoBehaviour, IFightComponent, IMaterialDescriptionProvider
{
    [SerializeField] private FightSide appliesTo = FightSide.Player;
    [SerializeField] private float deltaAttack = 1f;
    [TextArea] [SerializeField] private string description = "战斗开始：攻击加值。";
    ...
    private void OnBattleStart(FightContext context)
    {
        ...
        ApplyTo(appliesTo, context)?.AddAttack(deltaAttack);
    }
